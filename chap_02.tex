...
Ce qui nous mène à le modéliser dans le tableau \ref{tab:backlog_part1} et \ref{tab:backlog_part2}:
\newpage
\begin{table}[h!]
\vspace*{-1.5cm}
...
\item \textbf{Github}:Dont le logo est exposé dans la figure \ref{fig:gituhbl}, 
 est une plateforme collaborative basée sur Git, utilisée pour héberger du code, suivre les changements et faciliter le travail d’équipe sur des projets de développement logiciel \cite{ref25}.
 \begin{figure}[h]
    \centering
...
\end{figure}
\item \textbf{Visual Studio Code (VSCode)}:Représenté dans la figure \ref{fig:vscode}, VSCode \cite{26} est un éditeur de code source léger et extensible, populaire pour le développement Angular. Il offre une riche palette d’extensions, telles que des outils de linting, de débogage, et des intégrations Git, ce qui le rend idéal pour travailler sur des applications JavaScript et TypeScript, comme celles développées avec Angular.
 \begin{figure}[h]
    \centering
...
\end{figure}
 \subsection{Backend}
\item \textbf{Spring Boot}:Représenté dans la figure \ref{fig:SpringBoot}, Spring Boot est un framework Java qui facilite le développement d’applications Spring en simplifiant la configuration et le déploiement, tout en offrant des outils prêts à l’emploi comme un serveur embarqué et une sécurité intégrée \cite{ref16}.
 \begin{figure}[h]
    \centering
...
\end{figure}
\subsection{CI/CD}
\item \textbf{ArgoCD}:Représenté dans la figure \ref{fig:ArgoCD}, ArgoCD est un outil open-source d'intégration et de déploiement continu (CI/CD) spécialement conçu pour Kubernetes. Il permet de déployer et gérer des applications dans des environnements Kubernetes en suivant la méthodologie GitOps \cite{ref4}.
 \begin{figure}[h]
    \centering
...
\end{figure}
\subsection{GitOps}
\item \textbf{ArgoCD}:Représenté dans la figure \ref{fig:ArgoCD}, ArgoCD est un outil open-source spécialement conçu pour Kubernetes. Il permet de déployer et gérer des applications dans des environnements Kubernetes en suivant la méthodologie GitOps \cite{ref4}.
 \begin{figure}[h]
    \centering
    \includegraphics[width=1.6cm]{images/ArgoCD.png}
    \caption{Logo d'ArgoCD}
...
\item \textbf{MicroK8s}:Représenté dans la figure \ref{fig:kubernetes}, MicroK8s est une distribution légère et simplifiée de Kubernetes, développée par Canonical, conçue pour les déploiements sur des environnements à petite échelle, comme les postes de travail, les appareils IoT ou les clusters à nœud unique. Elle offre les principales fonctionnalités de Kubernetes, telles que l'orchestration de conteneurs, l'autoscaling et la gestion des ressources, tout en étant facile à installer et à configurer \cite{ref8}.
 \begin{figure}[h]
    \centering
